...
let gifNeedsUpdate = true;
let lastFilterFrame = 0;
...
gifBuffer = createGraphics(gif15.width, gif15.height);
gifCtx = gifBuffer.drawingContext;
}
...
gifBuffer.clear();
gifBuffer.image(gif15, 0, 0, gif15.width, gif15.height);
...
if (selectedArr === 15 && gif15) {
  ...
  let raw = map(focusDuration, 0, 20, 0, 1);
raw = constrain(raw, 0, 1);

// 👇 courbe douce (ease-in)
let distortion = pow(raw, 3.2);
distortion = constrain(distortion * 1.2, 0, 1);
...
if (distortion > 0.15 && frameCount - lastFilterFrame > 10) {
  constructionRGB(gifCtx, gifBuffer.canvas, {
    intensity: distortion * 0.6
  });
}

// =========================
// textureConcrete (moins fréquent)
// =========================
if (distortion > 0.4 && frameCount % 20 === 0) {
  textureConcrete(gifCtx, gifBuffer.canvas, {
    intensity: distortion * 0.7
  });
}
...
// fracture (très rare)
// =========================
if (distortion > 0.7 && frameCount % 40 === 0) {
  fracture(gifCtx, gifBuffer.canvas, {
...
tctx.drawImage(canvas, 0, 0);
  const pieces = 20 + (intensity * 40); // réduit drastiquement
  ...
  for (let i = 0; i < pieces; i++) {
    ...
    ctx.drawImage(temp, sx, sy, w, h, dx, dy, w, h);
  }
}