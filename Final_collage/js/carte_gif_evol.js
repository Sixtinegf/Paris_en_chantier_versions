...
if (selectedArr === 15 && gif15) {
  ...
  if (arr) {
    ...
    let gifDuration = 13000;

    if (!window.gifStart15 && alpha > 1) {
      window.gifStart15 = millis();
    }

    if (
      window.gifStart15 &&
      millis() - window.gifStart15 >= gifDuration
    ) {
      gifFinished15 = true;
    }
